...
const registerSchema = z.object({
  ...
  role: z.enum(['RETAILER', 'WHOLESALER', 'SALES_AGENT', 'LOGISTICS_AGENT']),
})
...
export async function shagoAuthRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get('/auth/me', { preHandler: [server.authenticate] }, async (request, reply) => {
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, email: true, phone: true, full_name: true, role: true, status: true, tier: true, avatar_url: true, is_verified: true, created_at: true },
    })
