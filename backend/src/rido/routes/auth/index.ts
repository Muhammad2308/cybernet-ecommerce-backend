...
const registerSchema = z.object({
  ...
  role: z.enum(['TRAVELER', 'LOGISTICS_ADMIN']),
  // TRAVELER picks independent or company — fleet join happens via invite
})
...
export async function ridoAuthRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/auth/login', async (request, reply) => {
    ...
    const token = signToken(server, { sub: user.id, email: user.email, role: user.role, fleet_company_id })
