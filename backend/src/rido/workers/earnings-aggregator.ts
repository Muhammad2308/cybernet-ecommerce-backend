...
export function startEarningsAggregator(): void {
  cron.schedule('0 1 * * *', async () => {
    ...
    try {
      ...
      console.log('[EarningsAggregator] complete')
    } catch (err) {
      console.error('[EarningsAggregator] failed:', err)
    }
  })
  console.log('[EarningsAggregator] scheduled (daily 01:00)')
}
...
async function aggregateDriverStats(from: Date, to: Date): Promise<void> {
  const earnings = await prisma.deliveryEarning.groupBy({
    ...
    _count: { id: true },
    _sum: { gross_fee: true, rido_commission_amount: true, driver_payout_amount: true },
  })
...
async function aggregateFleetStats(from: Date, to: Date): Promise<void> {
  const earnings = await prisma.deliveryEarning.groupBy({
    ...
    where: { fleet_company_id: { not: null }, created_at: { gte: from, lte: to } },
    _count: { id: true },
    _sum: { gross_fee: true, rido_commission_amount: true, fleet_payout_amount: true },
  })
  ...
  for (const e of earnings) {
    ...
    const activeDrivers = await prisma.deliveryEarning.groupBy({
      by: ['driver_id'],
      where: { fleet_company_id: e.fleet_company_id, created_at: { gte: from, lte: to } },
    })

    await prisma.fleetCompanyStats.upsert({
      where: { fleet_company_id_stat_date: { fleet_company_id: e.fleet_company_id, stat_date: from } },
      update: {
        total_deliveries: { increment: e._count.id },
