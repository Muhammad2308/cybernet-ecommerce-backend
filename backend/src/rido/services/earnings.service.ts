...
const DEFAULT_PLATFORM_COMMISSION_RATE = 10
...
export async function recordDeliveryEarning(deliveryId: string): Promise<void> {
  ...
  const delivery = await prisma.delivery.findUniqueOrThrow({
    where: { id: deliveryId },
    include: { traveler: { include: { fleet_membership: true } } },
  })
...
export async function getDriverEarningsSummary(driverId: string, from: Date, to: Date) {
  const earnings = await prisma.deliveryEarning.findMany({
    where: { driver_id: driverId, created_at: { gte: from, lte: to } },
  })
...
export async function getFleetEarningsSummary(fleetCompanyId: string, from: Date, to: Date) {
  const earnings = await prisma.deliveryEarning.findMany({
    where: { fleet_company_id: fleetCompanyId, created_at: { gte: from, lte: to } },
  })
  const currency = earnings[0]?.currency ?? 'NGN'
  return {
    total_deliveries: earnings.length,
