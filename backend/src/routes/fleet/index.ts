...
export async function fleetRoutes(server: FastifyInstance): Promise<void> {
  // ─── Public: Register a fleet company ───────────────────────
  server.post('/fleet/register', async (request, reply) => {
    ...
    const company = await registerFleetCompany(body.data).catch((err) => {
      if (err.code === 'P2002') {
        throw Object.assign(new Error('Registration number or contact email already registered'), { statusCode: 409 })
      }
      throw err
    })
  ...
  // ─── RIDO Admin: List all fleet companies ───────────────────
  server.get('/fleet', { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    const [companies, total] = await Promise.all([
      prisma.fleetCompany.findMany({
        ...
        skip: (page - 1) * limit,
        take: limit,
        include: { _count: { select: { driver_memberships: true } } },
  ...
  // ─── RIDO Admin: Approve fleet company ──────────────────────
  server.patch('/fleet/:fleetId/approve', { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    const company = await approveFleetCompany(fleetId)
    return reply.send({ success: true, data: company })
  })

  // ─── RIDO Admin: Suspend fleet company ──────────────────────
  server.patch('/fleet/:fleetId/suspend', { preHandler: [requireAdmin] }, async (request, reply) => {
    const { fleetId } = request.params as { fleetId: string }
    const company = await suspendFleetCompany(fleetId)
    return reply.send({ success: true, data: company })
  })

  // ─── RIDO Admin: Add fleet admin ────────────────────────────
  ...
  // ─── Fleet Admin: Get fleet overview ────────────────────────
  server.get('/fleet/:fleetId/overview', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    const { fleetId } = request.params as { fleetId: string }
    const overview = await getFleetOverview(fleetId)
    return reply.send({ success: true, data: overview })
  })

  // ─── Fleet Admin: Get drivers ────────────────────────────────
  server.get('/fleet/:fleetId/drivers', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    ...
    const drivers = await getFleetDrivers(fleetId, {
  ...
  server.patch('/fleet/:fleetId/drivers/:driverId/suspend', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    ...
    // Get fleet admins for notification
    const admins = await prisma.fleetCompanyAdmin.findMany({ where: { fleet_company_id: fleetId } })

    await publishEvent(RIDO_EVENTS.DRIVER_SUSPENDED, {
      driver_id: driverId,
      fleet_admin_ids: admins.map((a) => a.user_id),
    })

    return reply.send({ success: true, message: 'Driver suspended' })
  })

  // ─── Fleet Admin: Remove driver from fleet ──────────────────
  ...
  // ─── Fleet Admin: Create invite link ────────────────────────
  server.post('/fleet/:fleetId/invites', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    const { fleetId } = request.params as { fleetId: string }
    const invite = await createFleetInvite(fleetId, request.user.sub)
  ...
  server.post('/fleet/invites/:code/accept', { preHandler: [server.authenticate] }, async (request, reply) => {
    ...
    await publishEvent(RIDO_EVENTS.DRIVER_JOINED_FLEET, {
      driver_id: request.user.sub,
      driver_name: driver?.full_name ?? 'A driver',
      fleet_company_id: fleetCompanyId,
      fleet_admin_ids: admins.map((a) => a.user_id),
    })

    return reply.send({ success: true, message: 'Successfully joined fleet', data: { fleet_company_id: fleetCompanyId } })
  })

  // ─── Fleet Admin: Earnings summary ──────────────────────────
  server.get('/fleet/:fleetId/earnings', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    ...
    const summary = await getFleetEarningsSummary(fleetId, from, to)
    return reply.send({ success: true, data: summary })
  })

  // ─── Fleet Admin: Delivery history ──────────────────────────
    ...
    const [earnings, total] = await Promise.all([
      prisma.deliveryEarning.findMany({
        ...
        include: {
          delivery: {
            include: {
              shipment: { select: { pickup_address: true, dropoff_address: true } },
            },
          },
        },
        orderBy: { created_at: 'desc' },
  ...
  // ─── Fleet Admin: Live driver locations ─────────────────────
  server.get('/fleet/:fleetId/drivers/locations', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    const { fleetId } = request.params as { fleetId: string }

    const memberships = await prisma.fleetDriverMembership.findMany({
      where: { fleet_company_id: fleetId, status: 'ACTIVE' },
    ...
    const locations = await prisma.locationPing.findMany({
      where: {
        ...
        created_at: { gte: new Date(Date.now() - 30 * 1000) }, // last 30s
      },
      orderBy: { created_at: 'desc' },
      distinct: ['user_id'],
      include: {
  ...
  // ─── Fleet Admin: Webhook endpoint management ───────────────
  server.post('/fleet/:fleetId/webhooks', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    ...
    const endpoint = await prisma.webhookEndpoint.create({
      data: {
        fleet_company_id: fleetId,
        integration_name: 'FLEET',
        url: body.url,
        secret: body.secret,
        events: body.events,
      },
  ...
  server.get('/fleet/:fleetId/webhooks', { preHandler: [requireFleetAdmin] }, async (request, reply) => {
    const { fleetId } = request.params as { fleetId: string }
    const endpoints = await prisma.webhookEndpoint.findMany({ where: { fleet_company_id: fleetId } })
