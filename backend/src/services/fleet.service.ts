...
export async function addFleetAdmin(userId: string, fleetCompanyId: string) {
  // Ensure user exists and update their role
...
export async function getFleetOverview(fleetCompanyId: string) {
  ...
  const [totalDrivers, activeDrivers, deliveriesToday, pendingEarnings] = await Promise.all([
    ...
    prisma.fleetDriverMembership.count({
      where: { fleet_company_id: fleetCompanyId, status: FleetDriverStatus.ACTIVE },
    }),

    prisma.deliveryEarning.count({
      where: { fleet_company_id: fleetCompanyId, created_at: { gte: today } },
    }),
