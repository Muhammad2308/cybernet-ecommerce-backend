...
import { dispatchToIntegration } from './webhook.service'
...
export async function dispatchShagoJob(input: ShagoDispatchInput) {
  ...
  const existing = await prisma.shagoJob.findUnique({
    where: { shago_order_id: input.shago_order_id },
  })
  if (existing) {
    return { rido_job_id: existing.id, status: existing.status, duplicate: true }
  }
  ...
  const job = await prisma.shagoJob.create({
    data: {
      ...
      requested_delivery_window: input.requested_delivery_window
        ? new Date(input.requested_delivery_window)
        : null,
...
export async function getShagoJobStatus(ridoJobId: string) {
  ...
  return {
    rido_job_id: job.id,
    shago_order_id: job.shago_order_id,
    status: job.status,
    assigned_driver: job.delivery?.traveler
      ? { name: job.delivery.traveler.full_name, phone: job.delivery.traveler.phone }
      : null,
...
export async function confirmShagoDelivery(ridoJobId: string) {
  ...
  await prisma.$transaction([
    prisma.shagoJob.update({
      where: { id: ridoJobId },
      data: { status: ShagoJobStatus.DELIVERED },
    }),
    prisma.delivery.update({
      where: { id: job.rido_delivery_id },
      data: { status: 'DELIVERED', delivered_at: new Date() },
    }),
    prisma.transactionRoom.updateMany({
      where: { delivery_id: job.rido_delivery_id },
      data: { escrow_status: 'RELEASED' },
    }),
  ])

  // Trigger earnings calculation and event publication
  await publishEvent(RIDO_EVENTS.DELIVERY_CONFIRMED, {
    delivery_id: job.rido_delivery_id,
    shago_order_id: job.shago_order_id,
    rido_job_id: ridoJobId,
  })
...
export async function fileShagoDispute(
  ridoJobId: string,
  data: { reason: string; evidence_urls: string[]; disputed_amount: number },
) {
  const job = await prisma.shagoJob.findUnique({ where: { id: ridoJobId } })
  if (!job) throw new Error('Job not found')

  await prisma.shagoJob.update({
    where: { id: ridoJobId },
    data: { status: ShagoJobStatus.DISPUTED },
  })

  await fireShagoWebhook('job.disputed', job.shago_order_id, ridoJobId, {
    reason: data.reason,
...
export async function updateShagoJobStatus(
  ridoJobId: string,
  status: ShagoJobStatus,
  extra: Record<string, unknown> = {},
): Promise<void> {
  const job = await prisma.shagoJob.update({
    where: { id: ridoJobId },
    data: { status },
  })
  ...
  if (eventType) {
    await fireShagoWebhook(eventType, job.shago_order_id, ridoJobId, extra)
  }
}

async function fireShagoWebhook(
  eventType: string,
  shagoOrderId: string,
  ridoJobId: string,
  extra: Record<string, unknown>,
): Promise<void> {
  await dispatchToIntegration('SHAGO', null, eventType, {
    shago_order_id: shagoOrderId,
    rido_job_id: ridoJobId,
    ...extra,
  })
}
